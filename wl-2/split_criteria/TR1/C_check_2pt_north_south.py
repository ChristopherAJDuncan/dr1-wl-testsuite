...
# @Last Modified time: 2025-12-28 12:18:38
...
cata.loc[(cata['DEC'].values < 0), 'subsample_ID'] = 1
print(">>>>>>> subsample number ratio (north/south)", np.sum(cata['subsample_ID']==0)/np.sum(cata['subsample_ID']==1))
print(">>>>>>> subsample weight ratio (north/south)", np.sum(cata.loc[cata['subsample_ID']==0, 'shape_weight'].values)/np.sum(cata.loc[cata['subsample_ID']==1, 'shape_weight'].values))
...
## first check if it has already been measured
...
if os.path.exists(outpath):
    print('>>> 2pt for the whole sample already exists', outpath)
## loop over tomo bins
else:
    cata_final = []
...
for i_half in range(2):
    ...
    for i_ZBbin in range(Ntomo):
        ...
        for j_ZBbin in range(i_ZBbin, Ntomo):
            ## where to save
            outpath = os.path.join(outdir,
                                f'XI_{shear_type}_{save_label}_nbins_{theta_nbins}_theta_{theta_min:.1f}_{theta_max:.1f}_zbins_{i_ZBbin+1}_{j_ZBbin+1}.asc')
    ...
    cata_final = pd.concat(cata_final)
    outpath = os.path.join(outdir, 
                        f'XI_{shear_type}_{save_label}_nbins_{theta_nbins}_theta_{theta_min:.1f}_{theta_max:.1f}_combined.csv')
    cata_final.to_csv(outpath, index=False)
    print('combined cata saved to', outpath)
    outpath_2pt_list.append(outpath)
...
COLORs = ['k', 'orange', 'cyan']
LABELs = ['whole', 'north', 'south']
yscaling = 1e4
timesX = True
ylog = False
...
for which_half in ['xip', 'xim']:
    if which_half == 'xip':
        ...
        YLABEL = r'$\theta \times \Delta\xi_{-}$ [1e-4 arcmin]'
    outpath = f'./plots/C_north_south_{shear_type}_{which_half}_individual.png'
...
for which_half in ['xip', 'xim']:
    if which_half == 'xip':
        ...
        YLABEL = r'$\theta \times \Delta\xi_{-}$ [1e-4 arcmin]'
    outpath = f'./plots/C_north_south_{shear_type}_{which_half}_diff2base.png'
